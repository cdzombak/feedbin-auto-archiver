...
def test_basic_feed_rules():
    ...
    # Test uses_* methods
    assert rules.uses_keep_n(200) is True
    assert rules.uses_keep_n(100) is False
    assert rules.uses_max_age(100) is True
    assert rules.uses_max_age(200) is False  # uses keep_n instead
...
def test_basic_title_regex_rules():
    ...
    assert rules.uses_keep_n(100, "Newsletter") is True
    assert rules.uses_keep_n(100, "Daily News") is False
    assert rules.uses_max_age(100, "Daily News") is True
